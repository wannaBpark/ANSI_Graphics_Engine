...
namespace ansi
{
	...
	bool ShaderLoader::ParseShader(const std::string& filePath, std::vector<std::string>& sources)
	{
		...
		while (getline(fileStream, line)) {
			if (line.find("shader_vertex") != std::string::npos) {				/* "shader_vertex"가 등장할 경우, 버텍스 셰이더 단계로 변경 */
				stage = ShaderStage::Vertex;  
			} else if (line.find("shader_fragment") != std::string::npos) {		/* "shader_fragment"가 등장할 경우, 프래그먼트 셰이더 단계로 변경 */
				stage = ShaderStage::Fragment;
			} else {
				shaderStreams[static_cast<unsigned>(stage)] << line << '\n';		/* 현재 셰이더 단계에 대응되는 셰이더 코드 스트림에 파싱한 문자열을 추가 */
			}		
		}

		/* 완성된 셰이더 코드 스트림을 문자열로 저장 */
		for (unsigned i{ 0 }; i < static_cast<unsigned>(ShaderStage::length); ++i) {
